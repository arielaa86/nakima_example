...
<div class="card card-default card-table m-2">
    ...
    <div class="card-body">
        ...
        <div class="row m-4 justify-content-end">
            <p class="mt-4 ml-5" style="font-size: 16px">
                Ducos, le <g:formatDate date="${this.factureClient?.getDateAcquittee()}" format="dd MMMM yyyy"
        ...
        <div class="row justify-content-center m-2 mb-6">
            ...
            <div class="col-lg-8 print">
                ...
                <table class="table">
                    ...
                    <g:each var="paiement" in="${factureClient?.getPaiementsActives()}">
        ...
        <div class="row m-3 justify-content-center">
            ...
            <div class="col-lg-8">
                <g:if test="${this.factureClient.devis.montant == 0 && this.factureClient.devis.aRembourser < 0}">
                    <p class="mt-3" style="font-size: 20px; color: #1a7200">
                        <i class="fa fa-check-circle"></i>
                        Montant remboursé au client: ${new DecimalFormat("###,###.00 €").format(this.factureClient.devis.aRembourser * -1).replaceAll(",", " ")}
                    </p>
                </g:if>
            </div>
        </div>
        ...
        <div class="row m-5 justify-content-end noPrint">
            ...
            <g:if test="${user.authorities.contains(Role.findByAuthority("ROLE_DIRECTEUR"))}">
                <g:link class="btn btn-space btn-outline-secondary" controller="factureClient"
                        action="showFacture" id="${this.factureClient.id}">
                    <i class="fa fa-chevron-left"></i> Voir BDC
                </g:link>
            </g:if>

            <g:if test="${this.factureClient.devis.projet.client.email != null}">
                <g:form controller="factureClient" action="envoyerEmail" params="['id': this.factureClient.devis.id]"
...
<script>
    ...
    function sendMail() {
        ...
        var resp = confirm('Vous êtes sur le point d' + '\'envoyer la facture acquittée par email au client. Confirmez-vous l' + '\'envoi ?');

        if (resp) {
            showAlertWait('La facture acquittée est en cours d' + '\'envoi. Veuillez patienter', 15000);
            return true;
        }
